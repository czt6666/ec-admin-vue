<template>
  <div class="app-container" style="height: 857px;overflow-y: auto">
          ...
          <div v-else>
            <div v-for="menu in scope.row.menus" style="text-align: left">
              ...
              <el-tag v-for="perm in menu.permissions" :key="perm.permissionName" v-text="perm.permissionName"
                style="margin-right: 3px;" type="primary">{{perm.permissionName}}</el-tag>
            </div>
          </div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="管理" width="220">
        <template slot-scope="scope">
          <div v-if="scope.row.roleName !== '管理员'">
            ...
            <el-button v-permission="'role:delete'" type="danger" icon="delete" @click="removeRole(scope.$index)">
              删除
            </el-button>
          </div>
        </template>
          ...
          <div v-for=" (menu, _index) in allPermission" :key="menu.menuName">
            <span style="width: 100px;display: inline-block;">
              <el-button :type="isMenuNone(_index) ? '' : (isMenuAll(_index) ? 'success' : 'primary')" size="mini"
                style="width:80px;" @click="checkAll(_index)">{{ menu.menuName }}</el-button>
            </span>
  ...
  methods: {
    ...
    allPerm(_index) {
      ...
      for (let j = 0; j < menu.length; j++) {
        ...
        if (Math.round(menu[j].id / 100) === 9 && 901 !== menu[j].id) {
          this.addUnique(801, this.tempRole.permissions)
          this.addUnique(menu[j].id - 100, this.tempRole.permissions)
        }
      }
    },
    noPerm(_index) {
      //全部取消选中
      let menu = this.allPermission[_index].permissions;
      let editMenu = this.allPermission[6].permissions;
      for (let j = 0; j < menu.length; j++) {
        let idIndex = this.tempRole.permissions.indexOf(menu[j].id);
        if (idIndex > -1) {
          this.tempRole.permissions.splice(idIndex, 1);
        }
        /** 2024/1/8-ts-add: 在线编辑部分与预览部分取消选中联动，预览取消后若选中列表中包含编辑部分的权限则同样删除*/
        if (j < editMenu.length) {
          let editIdIndex = this.tempRole.permissions.indexOf(editMenu[j].id);
          if (editIdIndex > -1) {
            this.tempRole.permissions.splice(editIdIndex, 1);
          }
        }
    ...
    checkRequired(_perm, _index) {
      ...
      if (this.tempRole.permissions.indexOf(permId) > -1) {
        ...
        //那么就要将必选的权限勾上
        /** 2023/12/27-ts-add: 当9开头的权限被选中时也要选中对应8开头的权限*/
        if (Math.round(permId / 100) === 9) {
          this.addUnique(permId - 100, this.tempRole.permissions)
        }
    ...
    makeReuqiredPermissionChecked(_index) {
      //将本菜单必选的权限勾上
      let menu = this.allPermission[_index].permissions;
      if (this.allPermission[_index].menuName === "在线编辑") {
        this.addUnique(801, this.tempRole.permissions)
      }
      for (let j = 0; j < menu.length; j++) {
        let perm = menu[j];
